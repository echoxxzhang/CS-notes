...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    new_score = pow(3, player_score + opponent_score)
    list_score = list(str(new_score))
    if list_score[0] == list_score[len(list_score) - 1 ]:
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    the_score0, the_score1 = 0, 0  # 记录上一局的分数
    while (score0 < goal) and (score1 < goal):
        if who == 0:
            num_roll = strategy0(score0, score1)
            curr_score = take_turn(num_roll, score1, dice)
            score0 += curr_score
            if feral_hogs:
                if abs(num_roll - the_score0) == 2:
                    score0 += 3
                the_score0 = curr_score

        if who == 1:

            num_roll = strategy1(score1, score0)
            curr_score = take_turn(num_roll, score0, dice)
            score1 += curr_score
            if feral_hogs:
                if abs(num_roll - the_score1) == 2:
                    score1 += 3
                the_score1 = curr_score
